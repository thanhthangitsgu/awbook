...
const PromotionPage = () => {
    ...
    useEffect(() => {
        dispatch(allAPI.promotionAPI.getAll());
    }, []);

    //Lấy danh sách vào list
    useEffect(() => {
        promotionReducer.data && promotionReducer.data.data && setlistPromotion(promotionReducer.data.data);
    }, [promotionReducer])
    ...
    //Xử lí lọc
    useEffect(() => {
        let data = listPromotion;
        data = filter != "" ? data.filter(item => item.name.includes(filter)) : listPromotion;
            ...
            <div className="page-admin-content">
                ...
                <div className="bodyadmin">
                    <table className="table">
                        ...
                        <tbody>
                            {dataPromotion.length > 0 && dataPromotion.map((element, index) => {
                                return (
                                    <tr className={index % 2 ? "row-ood" : "row-even"} key={index}>
                                        ...
                                        <td className="col-last">
                                            <div className="action">
                                                <Link to={'/admin/khuyen-mai/' + element.id}><button className="btn-detail" >
