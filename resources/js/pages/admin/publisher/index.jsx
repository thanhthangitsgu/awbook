...
const PublisherPage = () => {
    ...
    useEffect(() => {
        publisherReducer.data && publisherReducer.data.data && setlistPublisher(publisherReducer.data.data);
    }, [publisherReducer])
    ...
    //Xử lí lọc
    useEffect(() => {
        let data = listPublisher;
        data = filter != "" ? data.filter(item => item.name.includes(filter)) : listPublisher;
        setdataPublisher(data);
    }, [filter])
        ...
        <div className="page-admin-main">
            ...
            <div className="page-admin-content">
                <div className="header">
                    <div className="title">QUẢN LÝ NHÀ XUẤT BẢN</div>
                ...
                <div className="bodyadmin">
                    <table className="table">
                        ...
                        <tbody>
                            {dataPublisher.length > 0 && dataPublisher.map((element, index) => {
                                return (
                                    <tr className={index % 2 ? "row-ood" : "row-even"} key={index}>
                                        <td className="col-first">{index + 1}</td>
                                        <td>{element.id_pub}</td>
                                        <td>{element.name}</td>
                                        <td>{element.address}</td>
                                        <td className="col-last">
                                            <div className="action">
                                                <Link to={'/admin/nha-xuat-ban/' + element.id_pub}><button className="btn-detail" >
                                                ...
                                                <button className="btn-delete" onClick={() => handleDelete(element.id_pub)}>
...
export default React.memo(PublisherPage);