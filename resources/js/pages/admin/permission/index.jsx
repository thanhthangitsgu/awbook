...
const PermissionPage = () => {
    ...
    useEffect(() => {
        dispatch(allAPI.permissionAPI.getAll());
    }, []);

    //Lấy danh sách vào list
    useEffect(() => {
        permissionReducer.data && permissionReducer.data.data && setlistPermission(permissionReducer.data.data);
    }, [permissionReducer])
    ...
    //Xử lí lọc
    useEffect(() => {
        let data = listPermission;
        data = filter != "" ? data.filter(item => item.name.includes(filter)) : listPermission;
        ...
        <div className="page-admin-main">
            ...
            <div className="page-admin-content">
                <div className="header">
                    ...
                    <div className="button-add">
                        <Link to="add"><button>{svg.btnAdd} Sửa</button></Link>
                    </div>
                ...
                <div className="bodyadmin">
                    <table className="table">
                        ...
                        <tbody>
                            {dataPermission.length > 0 && dataPermission.map((element, index) => {
                                return (
                                    <tr className={index % 2 ? "row-ood" : "row-even"} key={index}>
                                        ...
                                        <td className="col-last">
                                            <div className="action">
                                                <Link to={'/admin/quyen/' + element.id}><button className="btn-detail" >
...
export default React.memo(PermissionPage)